...
class ExtremeValueDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/extreme-value-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        ExtremeValueDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, ExtremeValueDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(ExtremeValueDistributionCase testCase) {
        ExtremeValueDistribution distribution = new ExtremeValueDistribution(testCase.location, testCase.scale);
    ...
    public static final class ExtremeValueDistributionParityFixture {
        public FixtureMeta meta;
        public List<ExtremeValueDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class ExtremeValueDistributionCase {
        public String name;
