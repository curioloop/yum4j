...
class ExponentialDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/exponential-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        ExponentialDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, ExponentialDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(ExponentialDistributionCase testCase) {
        ExponentialDistribution distribution = new ExponentialDistribution(testCase.lambda);
    ...
    public static final class ExponentialDistributionParityFixture {
        public FixtureMeta meta;
        public List<ExponentialDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class ExponentialDistributionCase {
        public String name;
