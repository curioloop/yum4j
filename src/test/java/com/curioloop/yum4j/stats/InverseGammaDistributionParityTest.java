...
class InverseGammaDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/inverse-gamma-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        InverseGammaDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, InverseGammaDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(InverseGammaDistributionCase testCase) {
        InverseGammaDistribution distribution = new InverseGammaDistribution(testCase.shape, testCase.scale);
    ...
    public static final class InverseGammaDistributionParityFixture {
        public FixtureMeta meta;
        public List<InverseGammaDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class InverseGammaDistributionCase {
        public String name;
        public double shape;
        public double scale;
        public double x;
