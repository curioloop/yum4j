...
class LogNormalDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/log-normal-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        LogNormalDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, LogNormalDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(LogNormalDistributionCase testCase) {
        LogNormalDistribution distribution = new LogNormalDistribution(testCase.location, testCase.scale);
    ...
    public static final class LogNormalDistributionParityFixture {
        public FixtureMeta meta;
        public List<LogNormalDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class LogNormalDistributionCase {
        public String name;
