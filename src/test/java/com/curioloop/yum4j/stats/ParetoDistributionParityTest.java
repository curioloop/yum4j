...
class ParetoDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/pareto-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        ParetoDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, ParetoDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(ParetoDistributionCase testCase) {
        ParetoDistribution distribution = new ParetoDistribution(testCase.scale, testCase.shape);
    ...
    public static final class ParetoDistributionParityFixture {
        public FixtureMeta meta;
        public List<ParetoDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class ParetoDistributionCase {
        public String name;
        public double scale;
        public double shape;
        public double x;
