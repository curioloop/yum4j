...
class UniformDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/uniform-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        UniformDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, UniformDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(UniformDistributionCase testCase) {
        UniformDistribution distribution = new UniformDistribution(testCase.lower, testCase.upper);
    ...
    public static final class UniformDistributionParityFixture {
        public FixtureMeta meta;
        public List<UniformDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class UniformDistributionCase {
        public String name;
        public double lower;
        public double upper;
        public double x;
