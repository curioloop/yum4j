...
class WeibullDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/weibull-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        WeibullDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, WeibullDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(WeibullDistributionCase testCase) {
        WeibullDistribution distribution = new WeibullDistribution(testCase.shape, testCase.scale);
    ...
    public static final class WeibullDistributionParityFixture {
        public FixtureMeta meta;
        public List<WeibullDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class WeibullDistributionCase {
        public String name;
        public double shape;
        public double scale;
        public double x;
