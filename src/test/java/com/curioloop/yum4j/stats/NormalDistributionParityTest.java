...
class NormalDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/normal-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        NormalDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, NormalDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(NormalDistributionCase testCase) {
        NormalDistribution distribution = new NormalDistribution(testCase.mean, testCase.standardDeviation);
    ...
    public static final class NormalDistributionParityFixture {
        public FixtureMeta meta;
        public List<NormalDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class NormalDistributionCase {
        public String name;
        public double mean;
