...
class NegativeBinomialDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/negative-binomial-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        NegativeBinomialDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, NegativeBinomialDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(NegativeBinomialDistributionCase testCase) {
        NegativeBinomialDistribution distribution = new NegativeBinomialDistribution(testCase.successes, testCase.successFraction);
        ...
        assertClose(
            testCase.name + " cdf roundtrip quantile",
            testCase.x,
            distribution.quantile(distribution.cdf(testCase.x)),
    ...
    public static final class NegativeBinomialDistributionParityFixture {
        public FixtureMeta meta;
        public List<NegativeBinomialDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class NegativeBinomialDistributionCase {
        public String name;
