...
class BetaDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/beta-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        BetaDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, BetaDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(BetaDistributionCase testCase) {
        BetaDistribution distribution = new BetaDistribution(testCase.alpha, testCase.beta);
    ...
    public static final class BetaDistributionParityFixture {
        public FixtureMeta meta;
        public List<BetaDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class BetaDistributionCase {
        public String name;
        public double alpha;
        public double beta;
        public double x;
