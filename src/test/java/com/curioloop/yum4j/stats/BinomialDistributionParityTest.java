...
class BinomialDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/binomial-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        BinomialDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, BinomialDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(BinomialDistributionCase testCase) {
        BinomialDistribution distribution = new BinomialDistribution(testCase.trials, testCase.successProbability);
        ...
        assertClose(
            ...
            testCase.x,
            distribution.quantile(testCase.cdf),
            testCase.quantileTolerance
        );
    ...
    public static final class BinomialDistributionParityFixture {
        public FixtureMeta meta;
        public List<BinomialDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class BinomialDistributionCase {
        public String name;
        public int trials;
