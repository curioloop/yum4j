...
class InverseGaussianDistributionParityTest {

    private static final String FIXTURE_PATH = "boost/math/inverse-gaussian-distribution-boost-parity-fixture.json";

    @TestFactory
    Stream<DynamicTest> boostReferenceValuesMatch() {
        InverseGaussianDistributionParityFixture fixture = ParityFixtureLoader.load(FIXTURE_PATH, InverseGaussianDistributionParityFixture.class);
        return fixture.cases.stream().map(testCase -> dynamicTest(testCase.name, () -> assertCase(testCase)));
    }

    private static void assertCase(InverseGaussianDistributionCase testCase) {
        InverseGaussianDistribution distribution = new InverseGaussianDistribution(testCase.mean, testCase.scale);
    ...
    public static final class InverseGaussianDistributionParityFixture {
        public FixtureMeta meta;
        public List<InverseGaussianDistributionCase> cases = Collections.emptyList();
    }
    ...
    public static final class InverseGaussianDistributionCase {
        public String name;
        public double mean;
        public double scale;
        public double x;
