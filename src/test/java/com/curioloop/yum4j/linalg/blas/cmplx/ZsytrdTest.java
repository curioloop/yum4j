...
class ZsytrdTest {
    ...
    private static final double[] SYMMETRIC_A = {
    ...
    private static void assertOffsetReductionMatchesDirect(BLAS.Uplo uplo) {
        ...
        double[] direct = SYMMETRIC_A.clone();
        ...
        assertEquals(0, Zsytrd.zsytrd(uplo, n, direct.clone(), 0, n, dDirect.clone(), 0, eDirect.clone(), 0, tauDirect.clone(), 0, query, 0, -1));
        int lwork = (int) query[0];
        double[] workDirect = new double[lwork];
        double[] workOffset = new double[lwork];

        assertEquals(0, Zsytrd.zsytrd(uplo, n, direct, 0, n, dDirect, 0, eDirect, 0, tauDirect, 0, workDirect, 0, lwork));
        assertEquals(0, Zsytrd.zsytrd(uplo, n, padded, aOff, lda, dOffset, 0, eOffset, 0, tauOffset, 0, workOffset, 0, lwork));
