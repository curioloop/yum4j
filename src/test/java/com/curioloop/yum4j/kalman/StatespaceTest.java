...
public class StatespaceTest {
    ...
    private static StateSpaceModel buildSharedAR1Default() {
        return StateSpaceModel.builder(1, 1, 1, 5)
    ...
    void testUnivariateFilter() {
        ModelFixture rep = buildAR1();
        FilterResult rConv = KalmanFilter.filter(rep, StateInitialization.approximateDiffuse());
        FilterResult rUni = UnivariateFilter.filter(rep, StateInitialization.approximateDiffuse());
    ...
    void testStationaryInit() {
        ...
        StateInitialization result = StateInitialization.stationary(rep);
        double expectedP0 = 4.0 / 3.0;
        assertEquals(expectedP0, result.initialStateCov()[0], TOL);
        assertEquals(0.0, result.initialState()[0], TOL);

        StateInitialization repeated = StateInitialization.stationary(rep);
    ...
    void testStationaryInitUsesMeanInFilters() {
        ...
        StateInitialization stationaryInit = StateInitialization.stationary(rep);
        ...
        FilterResult kalmanStationary = KalmanFilter.filter(rep, stationaryInit);
        FilterResult kalmanKnown = KalmanFilter.filter(rep, knownInit);
        FilterResult univariateStationary = UnivariateFilter.filter(rep, stationaryInit);
        FilterResult univariateKnown = UnivariateFilter.filter(rep, knownInit);
    ...
    void testMemoryControl() {
        ModelFixture rep = buildAR1();
        FilterResult r = KalmanFilter.filter(rep, StateInitialization.approximateDiffuse(), null,
    ...
    void testConcreteDefaultModelMatchesFixture() {
        StateSpaceModel direct = buildSharedAR1Default();
        ModelFixture fixture = buildAR1();

        FilterResult directResult = KalmanFilter.filter(direct, StateInitialization.approximateDiffuse());
        FilterResult fixtureResult = KalmanFilter.filter(fixture, StateInitialization.approximateDiffuse());
    ...
    void testSharedBuilderReusesStaticBlocks() {
        StateSpaceModel rep = buildSharedAR1Default();
    ...
    void testComplexDefaultModelLayout() {
        StateSpaceModel rep = StateSpaceModel.complex(1, 2, 1, 3);
