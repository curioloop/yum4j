...
public class ZKalmanMemoryBenchmark {
    ...
    private ZKalmanFilter.Pool borrowedFilterPool;
    private ZKalmanSmoother.Pool borrowedSmootherPool;
    private ZFilterResult smootherFilterInput;
    private ZKalmanSmoother.Pool missingBorrowedSmootherPool;
    ...
    public void setup() {
        ...
        borrowedFilterPool = new ZKalmanFilter.Pool();
        borrowedSmootherPool = new ZKalmanSmoother.Pool();
        missingBorrowedSmootherPool = new ZKalmanSmoother.Pool();

        ZKalmanFilter.filter(rep, init, borrowedFilterPool, fullFilterSpec);
        smootherFilterInput = ZKalmanFilter.filter(rep, init, null, smootherFilterSpec);
        ZKalmanSmoother.smooth(rep, smootherFilterInput, borrowedSmootherPool, fullSmootherSpec);
        missingSmootherFilterInput = ZKalmanFilter.filter(missingRep, init, null, smootherFilterSpec);
        ZKalmanSmoother.smooth(missingRep, missingSmootherFilterInput, missingBorrowedSmootherPool, fullSmootherSpec);
    ...
    public ZFilterResult filterBorrowedResultFull() {
        return ZKalmanFilter.filter(rep, init, borrowedFilterPool, fullFilterSpec);
    }

    @Benchmark
    public ZSmootherResult smoothBorrowedResultFull() {
        return ZKalmanSmoother.smooth(rep, smootherFilterInput, borrowedSmootherPool, fullSmootherSpec);
    }

    @Benchmark
    public ZSmootherResult smoothBorrowedResultFullMissing() {
        return ZKalmanSmoother.smooth(missingRep, missingSmootherFilterInput, missingBorrowedSmootherPool, fullSmootherSpec);
    }
