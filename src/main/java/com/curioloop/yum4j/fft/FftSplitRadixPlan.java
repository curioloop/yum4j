 ...
 * Ported from JTransforms (Ooura FFT implementation).
 */
final class FftSplitRadixPlan {
    ...
    FftSplitRadixPlan(int length) {
        if (length < 1) throw new IllegalArgumentException("zero-length FFT requested");
        if ((length & (length - 1)) != 0) throw new IllegalArgumentException("length must be a power of 2");
        this.length = length;
        if (length == 1) {
            this.ip = new int[0];
            this.w = new double[0];
        ...
        int twon = 2 * length;
        this.ip = new int[2 + (int) Math.ceil(2 + Math.sqrt(length / 2.0))];
        this.nw = twon >> 2;
        this.nc = length >> 2;
        this.w = new double[nw + nc];
        makewt(nw, ip, w);
        makect(nc, w, nw, ip);
        this.workspaceRequirement = FftWorkspace.Requirement.empty();
    }
    ...
    void exec(double[] data, int dataOffset, double factor, boolean forward) {
        ...
        int n2 = 2 * length;
        if (forward) {
            cftbsub(n2, data, dataOffset, ip, nw, w);
        } else {
            cftfsub(n2, data, dataOffset, ip, nw, w);
        }
        if (factor != 1.0) {
            for (int i = 0; i < n2; i++) {
    ...
    void exec(double[] data, int dataOffset, double factor, boolean forward, FftWorkspace workspace) {
        exec(data, dataOffset, factor, forward);
    }
    ...
    // ========== Ported static methods from JTransforms CommonUtils ==========
    ...
    private static void makect(int nc, double[] c, int startc, int[] ip) {
        int j, nch;
        double delta, deltaj;
    ...
    static void cftfsub(int n, double[] a, int offa, int[] ip, int nw, double[] w) {
        if (n > 8) {
            if (n > 32) {
    ...
    static void cftbsub(int n, double[] a, int offa, int[] ip, int nw, double[] w) {
        if (n > 8) {
            if (n > 32) {
    ...
    private static void cftf1st(int n, double[] a, int offa, double[] w, int startw) {
    ...
    private static void cftb1st(int n, double[] a, int offa, double[] w, int startw) {
        int j0, j1, j2, j3, k, m, mh;
        double wn4r, csc1, csc3, wk1r, wk1i, wk3r, wk3i, wd1r, wd1i, wd3r, wd3i;
        double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i, y0r, y0i, y1r, y1i, y2r, y2i, y3r, y3i;
        int idx0, idx1, idx2, idx3, idx4, idx5;
    ...
    private static void cftrec4(int n, double[] a, int offa, int nw, double[] w) {
        int isplt, j, k, m;
    ...
    private static int cfttree(int n, int j, int k, double[] a, int offa, int nw, double[] w) {
        int i, isplt, m;
        int idx1 = offa - n;
        if ((k & 3) != 0) {
            isplt = k & 1;
    ...
    private static void cftleaf(int n, int isplt, double[] a, int offa, int nw, double[] w) {
        if (n == 512) {
    ...
    private static void cftfx41(int n, double[] a, int offa, int nw, double[] w) {
        if (n == 128) {
            cftf161(a, offa, w, nw - 8);
            cftf162(a, offa + 32, w, nw - 32);
            cftf161(a, offa + 64, w, nw - 8);
    ...
    private static void cftmdl1(int n, double[] a, int offa, double[] w, int startw) {
        int j0, j1, j2, j3, k, m, mh;
    ...
    private static void cftmdl2(int n, double[] a, int offa, double[] w, int startw) {
    ...
    private static void cftf161(double[] a, int offa, double[] w, int startw) {
    ...
    private static void cftf162(double[] a, int offa, double[] w, int startw) {
    ...
    private static void cftf081(double[] a, int offa, double[] w, int startw) {
    ...
    private static void cftf082(double[] a, int offa, double[] w, int startw) {
    ...
    private static void cftf040(double[] a, int offa) {
        double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
    ...
    private static void cftb040(double[] a, int offa) {
        double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
    ...
    private static void cftxb020(double[] a, int offa) {
        double x0r, x0i;
    ...
    private static void bitrv2(int n, int[] ip, double[] a, int offa) {
        int j1, k1, l, m, nh, nm;
        double xr, xi, yr, yi;
        int idx0, idx1, idx2;
    ...
    private static void bitrv2conj(int n, int[] ip, double[] a, int offa) {
        int j1, k1, l, m, nh, nm;
        double xr, xi, yr, yi;
        int idx0, idx1, idx2;
        ...
        if (l == 8) {
            for (int k = 0; k < m; k++) {
                ...
                for (int j = 0; j < k; j++) {
                    ...
                    idx1 = offa + j1;
                    idx2 = offa + k1;
                    xr = a[idx1]; xi = -a[idx1 + 1]; yr = a[idx2]; yi = -a[idx2 + 1];
                    a[idx1] = yr; a[idx1 + 1] = yi; a[idx2] = xr; a[idx2 + 1] = xi;
                    j1 += nm; k1 += 2 * nm;
                    idx1 = offa + j1; idx2 = offa + k1;
                    xr = a[idx1]; xi = -a[idx1 + 1]; yr = a[idx2]; yi = -a[idx2 + 1];
                    a[idx1] = yr; a[idx1 + 1] = yi; a[idx2] = xr; a[idx2 + 1] = xi;
                    j1 += nm; k1 -= nm;
                    idx1 = offa + j1; idx2 = offa + k1;
    ...
    private static void bitrv216(double[] a, int offa) {
        double x1r, x1i, x2r, x2i, x3r, x3i, x4r, x4i, x5r, x5i, x7r, x7i, x8r, x8i, x10r, x10i, x11r, x11i, x12r, x12i, x13r, x13i, x14r, x14i;

        x1r = a[offa + 2]; x1i = a[offa + 3]; x2r = a[offa + 4]; x2i = a[offa + 5];
        x3r = a[offa + 6]; x3i = a[offa + 7]; x4r = a[offa + 8]; x4i = a[offa + 9];
        x5r = a[offa + 10]; x5i = a[offa + 11]; x7r = a[offa + 14]; x7i = a[offa + 15];
        x8r = a[offa + 16]; x8i = a[offa + 17]; x10r = a[offa + 20]; x10i = a[offa + 21];
        x11r = a[offa + 22]; x11i = a[offa + 23]; x12r = a[offa + 24]; x12i = a[offa + 25];
        x13r = a[offa + 26]; x13i = a[offa + 27]; x14r = a[offa + 28]; x14i = a[offa + 29];
        a[offa + 2] = x8r; a[offa + 3] = x8i; a[offa + 4] = x4r; a[offa + 5] = x4i;
        a[offa + 6] = x12r; a[offa + 7] = x12i; a[offa + 8] = x2r; a[offa + 9] = x2i;
        a[offa + 10] = x10r; a[offa + 11] = x10i; a[offa + 14] = x14r; a[offa + 15] = x14i;
        a[offa + 16] = x1r; a[offa + 17] = x1i; a[offa + 20] = x5r; a[offa + 21] = x5i;
        a[offa + 22] = x13r; a[offa + 23] = x13i; a[offa + 24] = x3r; a[offa + 25] = x3i;
        a[offa + 26] = x11r; a[offa + 27] = x11i; a[offa + 28] = x7r; a[offa + 29] = x7i;
    }

    private static void bitrv216neg(double[] a, int offa) {
        ...
        x1r = a[offa + 2]; x1i = a[offa + 3]; x2r = a[offa + 4]; x2i = a[offa + 5];
        x3r = a[offa + 6]; x3i = a[offa + 7]; x4r = a[offa + 8]; x4i = a[offa + 9];
        x5r = a[offa + 10]; x5i = a[offa + 11]; x6r = a[offa + 12]; x6i = a[offa + 13];
        x7r = a[offa + 14]; x7i = a[offa + 15]; x8r = a[offa + 16]; x8i = a[offa + 17];
        x9r = a[offa + 18]; x9i = a[offa + 19]; x10r = a[offa + 20]; x10i = a[offa + 21];
        x11r = a[offa + 22]; x11i = a[offa + 23]; x12r = a[offa + 24]; x12i = a[offa + 25];
        x13r = a[offa + 26]; x13i = a[offa + 27]; x14r = a[offa + 28]; x14i = a[offa + 29];
        x15r = a[offa + 30]; x15i = a[offa + 31];
        a[offa + 2] = x15r; a[offa + 3] = x15i; a[offa + 4] = x7r; a[offa + 5] = x7i;
        a[offa + 6] = x11r; a[offa + 7] = x11i; a[offa + 8] = x3r; a[offa + 9] = x3i;
        a[offa + 10] = x13r; a[offa + 11] = x13i; a[offa + 12] = x5r; a[offa + 13] = x5i;
        a[offa + 14] = x9r; a[offa + 15] = x9i; a[offa + 16] = x1r; a[offa + 17] = x1i;
        a[offa + 18] = x14r; a[offa + 19] = x14i; a[offa + 20] = x6r; a[offa + 21] = x6i;
        a[offa + 22] = x10r; a[offa + 23] = x10i; a[offa + 24] = x2r; a[offa + 25] = x2i;
        a[offa + 26] = x12r; a[offa + 27] = x12i; a[offa + 28] = x4r; a[offa + 29] = x4i;
        a[offa + 30] = x8r; a[offa + 31] = x8i;
    }

    private static void bitrv208(double[] a, int offa) {
        double x1r, x1i, x3r, x3i, x4r, x4i, x6r, x6i;

        x1r = a[offa + 2]; x1i = a[offa + 3]; x3r = a[offa + 6]; x3i = a[offa + 7];
        x4r = a[offa + 8]; x4i = a[offa + 9]; x6r = a[offa + 12]; x6i = a[offa + 13];
        a[offa + 2] = x4r; a[offa + 3] = x4i; a[offa + 6] = x6r; a[offa + 7] = x6i;
        a[offa + 8] = x1r; a[offa + 9] = x1i; a[offa + 12] = x3r; a[offa + 13] = x3i;
    }

    private static void bitrv208neg(double[] a, int offa) {
        double x1r, x1i, x2r, x2i, x3r, x3i, x4r, x4i, x5r, x5i, x6r, x6i, x7r, x7i;

        x1r = a[offa + 2]; x1i = a[offa + 3]; x2r = a[offa + 4]; x2i = a[offa + 5];
        x3r = a[offa + 6]; x3i = a[offa + 7]; x4r = a[offa + 8]; x4i = a[offa + 9];
        x5r = a[offa + 10]; x5i = a[offa + 11]; x6r = a[offa + 12]; x6i = a[offa + 13];
        x7r = a[offa + 14]; x7i = a[offa + 15];
        a[offa + 2] = x7r; a[offa + 3] = x7i; a[offa + 4] = x3r; a[offa + 5] = x3i;
        a[offa + 6] = x5r; a[offa + 7] = x5i; a[offa + 8] = x1r; a[offa + 9] = x1i;
        a[offa + 10] = x6r; a[offa + 11] = x6i; a[offa + 12] = x2r; a[offa + 13] = x2i;
        a[offa + 14] = x4r; a[offa + 15] = x4i;
    }

    // --- Real FFT support (for later use) ---

    static void rftfsub(int n, double[] a, int offa, int nc, double[] c, int startc) {
        int k, kk, ks, m;
    ...
    static void rftbsub(int n, double[] a, int offa, int nc, double[] c, int startc) {
        int k, kk, ks, m;
        double wkr, wki, xr, xi, yr, yi;
        int idx1, idx2;
