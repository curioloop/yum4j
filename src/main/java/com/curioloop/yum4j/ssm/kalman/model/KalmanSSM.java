...
public class KalmanSSM {
    ...
    public KalmanSSM(int kEndog, int kStates, int kPosdef, int nobs) {
    ...
    public static KalmanSSM complex(int kEndog, int kStates, int kPosdef, int nobs) {
        return new KalmanSSM(kEndog, kStates, kPosdef, nobs, COMPLEX_SCALAR_WIDTH,
    ...
    public final boolean sameLayoutAs(KalmanSSM other) {
        Objects.requireNonNull(other, "other");
        return kEndog == other.kEndog
            && kStates == other.kStates
            && kPosdef == other.kPosdef
            && nobs == other.nobs
            && scalarWidth == other.scalarWidth
            && designStride == other.designStride
            && obsInterceptStride == other.obsInterceptStride
            && obsCovStride == other.obsCovStride
            && transitionStride == other.transitionStride
            && stateInterceptStride == other.stateInterceptStride
            && selectionStride == other.selectionStride
            && stateCovStride == other.stateCovStride
            && endogBase == other.endogBase
            && endogStride == other.endogStride
            && sameLength(design, other.design)
            && sameLength(obsIntercept, other.obsIntercept)
            && sameLength(obsCov, other.obsCov)
            && sameLength(transition, other.transition)
            && sameLength(stateIntercept, other.stateIntercept)
            && sameLength(selection, other.selection)
            && sameLength(stateCov, other.stateCov)
            && sameLength(endog, other.endog)
            && sameLength(missing, other.missing)
            && sameLength(nmissing, other.nmissing);
    }

    public final void copyDataFromSameLayout(KalmanSSM source) {
        Objects.requireNonNull(source, "source");
        if (!sameLayoutAs(source)) {
            throw new IllegalArgumentException("source does not have the same KalmanSSM layout");
        }
        copy(source.design, design);
        copy(source.obsIntercept, obsIntercept);
        copy(source.obsCov, obsCov);
        copy(source.transition, transition);
        copy(source.stateIntercept, stateIntercept);
        copy(source.selection, selection);
        copy(source.stateCov, stateCov);
        copy(source.endog, endog);
        copy(source.missing, missing);
        copy(source.nmissing, nmissing);
    }
    ...
    private static boolean sameLength(double[] left, double[] right) {
        return left == null ? right == null : right != null && left.length == right.length;
    }

    private static boolean sameLength(boolean[] left, boolean[] right) {
        return left == null ? right == null : right != null && left.length == right.length;
    }

    private static boolean sameLength(int[] left, int[] right) {
        return left == null ? right == null : right != null && left.length == right.length;
    }
    ...
    private static double[] copyObservationVector(double[] source,
                                                  IntUnaryOperator offsetFn,
                                                  int width,
                                                  int nobs,
                                                  int scalarWidth) {
        double[] copy = new double[scalarWidth * width * nobs];
        for (int t = 0; t < nobs; t++) {
            System.arraycopy(source, offsetFn.applyAsInt(t), copy, t * scalarWidth * width, scalarWidth * width);
        }
        return copy;
    }
    ...
    private static double[] copySquareBlocks(double[] source,
        ...
        for (int t = 0; t < nobs; t++) {
            int sourceOffset = offsetFn.applyAsInt(t);
    ...
    public static final class Builder {
        ...
        public KalmanSSM build() {
            ...
            return new KalmanSSM(kEndog, kStates, kPosdef, nobs, scalarWidth,
                design, designStride,
