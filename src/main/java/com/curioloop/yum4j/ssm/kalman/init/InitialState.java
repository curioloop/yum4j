...
public final class InitialState {
    ...
    private InitialState[] blockInitializations;
    ...
    public static final class StationaryWorkspace {
        ...
        public void ensureCapacity(KalmanSSM model) {
    ...
    private InitialState(Kind kind,
        ...
        this.diffuseTolerance = validateDiffuseTolerance(diffuseTolerance);
    }
    ...
    public static InitialState known(double[] initialState, double[] initialStateCov) {
        validateKnownInputs(initialState, initialStateCov);
        return new InitialState(Kind.KNOWN, UNSPECIFIED_DIMENSION,
            initialState, initialStateCov, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState known(int dimension, double[] initialState, double[] initialStateCov) {
        validateCompositeDimension(dimension);
        validateKnownInputs(initialState, initialStateCov);
        return new InitialState(Kind.KNOWN, dimension,
            initialState, initialStateCov, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState diffuse() {
        return new InitialState(Kind.DIFFUSE, UNSPECIFIED_DIMENSION,
            null, null, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState diffuse(int dimension) {
        validateCompositeDimension(dimension);
        return new InitialState(Kind.DIFFUSE, dimension,
            null, null, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState approximateDiffuse() {
        return approximateDiffuse(UNSPECIFIED_DIMENSION, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState approximateDiffuse(double approximateDiffuseVariance) {
        return approximateDiffuse(UNSPECIFIED_DIMENSION, null, approximateDiffuseVariance);
    }

    public static InitialState approximateDiffuse(int dimension) {
        return approximateDiffuse(dimension, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState approximateDiffuse(int dimension, double approximateDiffuseVariance) {
        return approximateDiffuse(dimension, null, approximateDiffuseVariance);
    }

    public static InitialState approximateDiffuse(double[] initialState, double approximateDiffuseVariance) {
    ...
    public static InitialState approximateDiffuse(int dimension,
        ...
        return new InitialState(Kind.APPROXIMATE_DIFFUSE, dimension,
            initialState, null, null, approximateDiffuseVariance);
    }

    public static InitialState stationary(double[] initialState, double[] initialStateCov) {
        Objects.requireNonNull(initialState, "initialState");
        Objects.requireNonNull(initialStateCov, "initialStateCov");
        return new InitialState(Kind.STATIONARY, UNSPECIFIED_DIMENSION,
            initialState, initialStateCov, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState stationary(int dimension) {
        validateCompositeDimension(dimension);
        return new InitialState(Kind.STATIONARY, dimension,
            null, null, null, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    public static InitialState stationary(KalmanSSM model) {
        return stationary(model, 0);
    }

    public static InitialState stationary(KalmanSSM model, int t) {
        Objects.requireNonNull(model, "model");
        validateTimeIndex(model.observationCount(), t);
        InitialState solution = solveStationary(model, t);
        if (solution == null) {
    ...
    public InitialState set(InitialState initialization) {
        return set(0, dimension, initialization);
    }

    public InitialState set(int stateIndex, InitialState initialization) {
        return set(stateIndex, stateIndex + 1, initialization);
    }

    public InitialState set(int startInclusive, int endExclusive, InitialState initialization) {
        requireComposite();
    ...
    public InitialState unset(int stateIndex) {
        return unset(stateIndex, stateIndex + 1);
    }

    public InitialState unset(int startInclusive, int endExclusive) {
        requireComposite();
        validateRange(startInclusive, endExclusive, dimension);
    ...
    public InitialState withDiffuseTolerance(double diffuseTolerance) {
        InitialState next = new InitialState(kind, dimension, initialState, initialStateCov, initialDiffuseStateCov,
            approximateDiffuseVariance, diffuseTolerance);
        next.blockStarts = blockStarts == null ? null : Arrays.copyOf(blockStarts, blockStarts.length);
        next.blockEnds = blockEnds == null ? null : Arrays.copyOf(blockEnds, blockEnds.length);
        next.blockInitializations = blockInitializations == null
            ? null
            : Arrays.copyOf(blockInitializations, blockInitializations.length);
        next.blockCount = blockCount;
        return next;
    }

    public InitialState resolve(KalmanSSM model) {
        return resolve(model, 0, null);
    }

    public InitialState resolve(KalmanSSM model, int t) {
    ...
    private InitialState resolve(KalmanSSM model,
                                        int t,
        ...
        StationaryWorkspace resolvedWorkspace = mayResolveStationary()
            ...
            : stationaryWorkspace;
        InitialState resolved = kind == Kind.COMPOSITE
            ? resolveComposite(model, t, resolvedWorkspace)
            : resolveLeaf(model, t, resolvedWorkspace);
        return resolved.withDiffuseTolerance(diffuseTolerance);
    }
    ...
    public boolean resolveInto(KalmanSSM model,
                               int t,
                               double[] state,
                               int stateOffset,
                               double[] covariance,
    ...
    private static InitialState solveResolvedStationary(KalmanSSM stateSpace,
                                                               int t,
                                                               StationaryWorkspace stationaryWorkspace) {
    ...
    private static boolean solveResolvedStationaryInto(KalmanSSM stateSpace,
                                                       int t,
                                                       StationaryWorkspace stationaryWorkspace,
                                                       double[] state,
                                                       int stateOffset,
    ...
    private InitialState resolveComposite(KalmanSSM model,
                                                 int t,
                                                 StationaryWorkspace stationaryWorkspace) {
        int blockDimension = requireCompatibleDimension(model.stateCount());
    ...
    private InitialState resolveLeaf(KalmanSSM model,
                                            int t,
                                            StationaryWorkspace stationaryWorkspace) {
        int blockDimension = requireCompatibleDimension(model.stateCount());
        int scalarWidth = scalarWidth(model);
        ...
        switch (kind) {
            ...
            case STATIONARY:
                ...
                InitialState stationary = solveResolvedStationary(model, t, stationaryWorkspace);
                if (stationary == null) {
                    throw new IllegalArgumentException("State block is not discrete-stationary at t=" + t);
                }
                return stationary;
    ...
    private boolean resolveIntoTargets(KalmanSSM model,
                                       int t,
                                       StationaryWorkspace stationaryWorkspace,
                                       double[] state,
                                       int stateOffset,
        ...
        if (kind == Kind.COMPOSITE) {
            ...
            for (int i = 0; i < blockCount; i++) {
                ...
                int end = blockEnds[i];
                KalmanSSM blockModel = buildStateBlockModel(model, t, start, end - start);
    ...
    private void validateResolvedAgainstModel(KalmanSSM model) {
        int blockDimension = requireCompatibleDimension(model.stateCount());
        int scalarWidth = scalarWidth(model);
    ...
    public static InitialState resolved(int dimension,
                                                double[] initialState,
                                                double[] initialStateCov,
                                                double[] initialDiffuseStateCov) {
        return new InitialState(Kind.RESOLVED, dimension,
            initialState, initialStateCov, initialDiffuseStateCov, DEFAULT_APPROXIMATE_DIFFUSE_VARIANCE);
    }

    private static InitialState solveStationary(KalmanSSM stateSpace, int t) {
        ...
        return new InitialState(Kind.STATIONARY, stateSpace.stateCount(),
    ...
    private static int scalarWidth(KalmanSSM model) {
    ...
    private static double validateDiffuseTolerance(double diffuseTolerance) {
        if (!(diffuseTolerance > 0.0) || !Double.isFinite(diffuseTolerance)) {
            throw new IllegalArgumentException("diffuseTolerance must be finite and positive");
        }
        return diffuseTolerance;
    }
    ...
    private static void validateBlockDimension(InitialState initialization, int expectedDimension) {
    ...
    private void insertBlock(int startInclusive, int endExclusive, InitialState initialization) {
    ...
    private void ensureBlockCapacity(int capacity) {
        if (blockStarts == null || blockStarts.length < capacity) {
            ...
            blockInitializations = blockInitializations == null
                ? new InitialState[nextCapacity]
                : Arrays.copyOf(blockInitializations, nextCapacity);
        }
    }

    private static KalmanSSM buildStateBlockModel(KalmanSSM model,
                                                        int t,
                                                        int startState,
                                                        int blockDimension) {
        int scalarWidth = scalarWidth(model);
        ...
        KalmanSSM.Builder builder = model.complex()
            ? KalmanSSM.complexBuilder(1, blockDimension, kPosdef, 1)
            : KalmanSSM.builder(1, blockDimension, kPosdef, 1);
        return builder
    ...
    private static StationaryWorkspace prepareStationaryWorkspace(KalmanSSM model,
                                                                 StationaryWorkspace stationaryWorkspace) {
    ...
    public static int requiredStationaryBackingLength(KalmanSSM model) {
    ...
    public static int requiredStationaryPivotLength(KalmanSSM model) {
        return model.complex()
    ...
    public static final class Builder {
        private final InitialState composite;
        private double diffuseTolerance = FilterOptions.DEFAULT_DIFFUSE_TOLERANCE;

        private Builder(int dimension) {
            this.composite = new InitialState(dimension);
        }

        public Builder known(int startInclusive,
                             int endExclusive,
                             double[] initialState,
                             double[] initialStateCov) {
            composite.set(startInclusive, endExclusive,
                InitialState.known(endExclusive - startInclusive, initialState, initialStateCov));
            return this;
        }

        public Builder diffuse(int startInclusive, int endExclusive) {
            composite.set(startInclusive, endExclusive,
                InitialState.diffuse(endExclusive - startInclusive));
            return this;
        }

        public Builder approximateDiffuse(int startInclusive, int endExclusive) {
            composite.set(startInclusive, endExclusive,
                InitialState.approximateDiffuse(endExclusive - startInclusive));
            return this;
        }

        public Builder approximateDiffuse(int startInclusive, int endExclusive, double variance) {
            composite.set(startInclusive, endExclusive,
                InitialState.approximateDiffuse(endExclusive - startInclusive, variance));
            return this;
        }

        public Builder stationary(int startInclusive, int endExclusive) {
            composite.set(startInclusive, endExclusive,
                InitialState.stationary(endExclusive - startInclusive));
            return this;
        }

        public Builder stationary(int startInclusive,
                                  int endExclusive,
                                  double[] initialState,
                                  double[] initialStateCov) {
            composite.set(startInclusive, endExclusive,
                InitialState.stationary(initialState, initialStateCov));
            return this;
        }
        ...
        public InitialState build() {
            return composite.withDiffuseTolerance(diffuseTolerance);
        }
    }
}