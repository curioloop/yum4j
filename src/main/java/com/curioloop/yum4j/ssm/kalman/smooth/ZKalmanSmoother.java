...
public final class ZKalmanSmoother {
    ...
    private static final int SMOOTHER_STATE_AUTOCOV = 0x10;
    private static final int SMOOTHER_ALL = 0x0F;

    private static boolean storesAuxiliary(int smootherOutput) {
     ...
     */
    static ZSmootherResult smooth(KalmanSSM model,
                                  ZFilterResult fr,
                                  Pool pool,
                                  SmootherOptions spec) {
        requireComplexInputs(model, model);
    ...
    static ZSmootherResult smoothInto(KalmanSSM stateSpace,
                                      KalmanSSM observationModel,
                                      ZFilterResult fr,
    ...
    private static void computeConventionalStateAutocovariance(ZFilterResult fr,
                                                               ZSmootherResult result,
                                                               int t,
                                                               int kStates,
                                                               double[] scratch,
                                                               int lBase,
        ...
        ZLAS.zgemm(BLAS.Trans.NoTrans, BLAS.Trans.NoTrans, kStates, kStates, kStates,
            ...
            0.0, 0.0, result.smoothedStateAutocovariance, autocovOff >> 1, kStates);
        ZLAS.zgemm(BLAS.Trans.NoTrans, BLAS.Trans.NoTrans, kStates, kStates, kStates,
            1.0, 0.0, scratch, nBase >> 1, kStates,
            result.smoothedStateAutocovariance, autocovOff >> 1, kStates,
            0.0, 0.0, scratch, tmpBase >> 1, kStates);
        ZLAS.zgemm(BLAS.Trans.NoTrans, BLAS.Trans.NoTrans, kStates, kStates, kStates,
            -1.0, 0.0, fr.predictedStateCov, nextPredCovOff >> 1, kStates,
            scratch, tmpBase >> 1, kStates,
            1.0, 0.0, result.smoothedStateAutocovariance, autocovOff >> 1, kStates);
    }

    private static ZSmootherResult smoothInternal(KalmanSSM stateSpace,
                                                  KalmanSSM observationModel,
                                                  ZFilterResult fr,
                                                  Pool pool,
        ...
        if ((smoothMethod & KalmanSmoother.SMOOTH_UNIVARIATE) != 0 && !fr.perObsKalmanGain) {
            throw new IllegalArgumentException("Complex univariate smoothing requires a univariate ZFilterResult");
        }
        if ((smootherOutput & SMOOTHER_STATE_AUTOCOV) != 0
                && (smoothMethod & KalmanSmoother.SMOOTH_CONVENTIONAL) == 0) {
            throw new UnsupportedOperationException(
                "Complex state autocovariance smoothing is implemented for conventional smoothing only");
        }
    ...
    private static ZSmootherResult smoothClassical(KalmanSSM stateSpace,
                                                   KalmanSSM observationModel,
                                                   ZFilterResult fr,
                                                   Pool pool,
                                                   int smootherOutput,
        ...
        if (!storeAuxiliary && doDisturbance) {
            pool.ensurePooledScratch(kEndog);
        }

        ZSmootherResult result = prepareResult(pool, target, reuseResult,
            kEndog, kStates, kPosdef, nobs,
            doState, doStateCov, false, doDisturbance, doDisturbanceCov, storeAuxiliary);
        ...
        for (int t = nobs - 1; t >= 0; t--) {
            ...
            if (t < nobs - 1) {
                ...
                if (needSmoothedStateCov) {
                    ...
                    transposeInPlace(scratch, nBase, kStates);
                    ZLAS.zpotrs(BLAS.Uplo.Lower, kStates, kStates,
                        scratch, tmpJBase >> 1, kStates, scratch, nBase >> 1, kStates);
                    symmetrizeComplexSymmetric(scratch, nBase, kStates);
                }
            } else {
            ...
            if (ke == 0) {
                ...
                if (needSmoothedStateCov) {
                    ...
                    symmetrizeComplexSymmetric(smoothedStateCov, smoothedStateCovOff, kStates);
                }
                ...
                if (doDisturbance && kPosdef > 0) {
                    ...
                    ZLAS.zgemv(BLAS.Trans.Trans, kStates, kPosdef, 1.0, 0.0,
                ...
                if (doDisturbanceCov) {
                    ...
                    if (kPosdef > 0) {
                        ...
                        ZLAS.zgemm(BLAS.Trans.Trans, BLAS.Trans.NoTrans, kPosdef, kPosdef, kStates,
            ...
            if (smoothingError != null) {
                ...
                ZLAS.zgemv(BLAS.Trans.Trans, kStates, ke, -1.0, 0.0,
                    K, kOff, kLd,
                    scratch, rBase, 1,
            ...
            if (doDisturbanceCov) {
                ...
                if (ke >= kEndog) {
                    ZLAS.zgemm(BLAS.Trans.Trans, BLAS.Trans.NoTrans, ke, ke, kStates,
    ...
    private static ZSmootherResult smoothAlternative(KalmanSSM stateSpace,
                                                     KalmanSSM observationModel,
                                                     ZFilterResult fr,
                                                     Pool pool,
                                                     int smootherOutput,
        ...
        for (int t = nobs - 1; t >= 0; t--) {
            ...
            if (doStateCov) {
                ...
                symmetrizeComplexSymmetric(result.smoothedStateCov, smoothedStateCovOff, kStates);
            }
            ...
            if (ke == 0) {
                ...
                ZLAS.zgemv(BLAS.Trans.Trans, kStates, kStates, 1.0, 0.0,
                    transition, stateSpace.transitionOffset(t), transitionLd,
                    scratch, tildeRBase, 1,
                    0.0, 0.0, scratch, hatRBase, 1);
                ZLAS.zgemm(BLAS.Trans.Trans, BLAS.Trans.NoTrans, kStates, kStates, kStates,
                    1.0, 0.0, transition, stateSpace.transitionOffset(t) >> 1, transitionLd,
            ...
            int predCovOff = fr.predictedStateCovOffset(t);
            ZLAS.zgemm(BLAS.Trans.Trans, BLAS.Trans.NoTrans, kStates, kStates, ke,
    ...
    private static ZSmootherResult smoothConventionalCore(KalmanSSM stateSpace,
                                                          KalmanSSM observationModel,
                                                          ZFilterResult fr,
                                                          Pool pool,
                                                          int smootherOutput,
    ...
    private static ZSmootherResult smoothDiffuse(KalmanSSM stateSpace,
                                                 KalmanSSM observationModel,
                                                 ZFilterResult fr,
                                                 Pool pool,
                                                 int smootherOutput,
            ...
            if (observationModel.missingCount(t) == kEndog) {
                ...
                for (int i = kEndog - 1; i >= 0; i--) {
                    ...
                    if (isDiffuse && fInf > toleranceDiffuse) {
                        ...
                        addWithTranspose(scratch, tmpPInfNInf2Base, scratch, nInf2Base, kStates);
        ...
        if (storeAuxiliary) {
            zero(result.scaledSmoothedEstimator, result.scaledSmoothedEstimatorOffset(nobs), 2 * kStates);
            zero(result.scaledSmoothedEstimatorCov, result.scaledSmoothedEstimatorCovOffset(nobs), ksKs);
            shiftLeft(result.scaledSmoothedEstimator, result.scaledSmoothedEstimatorBase(), 2 * kStates, nobs * 2 * kStates);
            shiftLeft(result.scaledSmoothedEstimatorCov, result.scaledSmoothedEstimatorCovBase(), ksKs, nobs * ksKs);
        }
        zero(result.scaledSmoothedDiffuseEstimator, result.scaledSmoothedDiffuseEstimatorOffset(nobs), 2 * kStates);
        zero(result.scaledSmoothedDiffuse1EstimatorCov, result.scaledSmoothedDiffuse1EstimatorCovOffset(nobs), ksKs);
        zero(result.scaledSmoothedDiffuse2EstimatorCov, result.scaledSmoothedDiffuse2EstimatorCovOffset(nobs), ksKs);
        ...
        if (doStateAutocov) {
            zero(result.smoothedStateAutocovariance, result.smoothedStateAutocovarianceBase(),
                result.smoothedStateAutocovarianceLength());
            computeDiffuseStateAutocovarianceTail(stateSpace, observationModel, fr, result, scratch,
                l0Base, l1Base, lBase, tmpTtNBase);
        }
    ...
    private static void computeDiffuseStateAutocovarianceTail(KalmanSSM stateSpace,
        ...
        double[] design = observationModel.designData();
        int designLd = observationModel.designLeadingDimension();
        double[] nextMeasuredN = new double[ksKs];
        double[] currentPredictedN = new double[ksKs];
        double[] measuredN = new double[ksKs];

        for (int t = observationModel.observationCount() - 1; t >= fr.nobsDiffuse; t--) {
            System.arraycopy(currentPredictedN, 0, measuredN, 0, ksKs);
            ...
            for (int obs = kEndog - 1; obs >= 0; obs--) {
                ...
                double fRe = fr.forecastErrorCov[foreErrCovOff + obs * 2 * kEndog + obs * 2];
                if (!(fRe > 0.0)) {
                    continue;
                }
                double fInv = 1.0 / fRe;
                setIdentity(scratch, singleObservationTransitionBase, kStates);
                int zRowOff = designOff + obs * 2 * designLd;
                for (int row = 0; row < kStates; row++) {
                    int gainOff = kgOff + row * 2 * kEndog + obs * 2;
                    double gainRe = fr.kalmanGain[gainOff];
                    double gainIm = fr.kalmanGain[gainOff + 1];
                    for (int col = 0; col < kStates; col++) {
                        int zOff = zRowOff + col * 2;
                        int lOff = row * 2 * kStates + col * 2;
                        subtractProduct(scratch, singleObservationTransitionBase + lOff,
                            gainRe, gainIm, design[zOff], design[zOff + 1]);
                    }
                }
                ZLAS.zgemm(BLAS.Trans.NoTrans, BLAS.Trans.NoTrans,
                    kStates, kStates, kStates,
                    1.0, 0.0, scratch, measurementTransitionBase >> 1, kStates,
                    scratch, singleObservationTransitionBase >> 1, kStates,
                    0.0, 0.0, scratch, tmpBase >> 1, kStates);
                ZLAS.zcopy(kStates * kStates, scratch, tmpBase, 1,
                    scratch, measurementTransitionBase, 1);

                ZLAS.zgemm(BLAS.Trans.NoTrans, BLAS.Trans.NoTrans,
                    kStates, kStates, kStates,
                    1.0, 0.0, measuredN, 0, kStates,
                    scratch, singleObservationTransitionBase >> 1, kStates,
                    0.0, 0.0, scratch, tmpBase >> 1, kStates);
                ZLAS.zgemm(BLAS.Trans.Trans, BLAS.Trans.NoTrans,
                    kStates, kStates, kStates,
                    1.0, 0.0, scratch, singleObservationTransitionBase >> 1, kStates,
                    scratch, tmpBase >> 1, kStates,
                    0.0, 0.0, measuredN, 0, kStates);
                addOuterConjZ(measuredN, 0, design, zRowOff, fInv, scratch, tmpBase, kStates);
            }

            ZLAS.zgemm(BLAS.Trans.NoTrans, BLAS.Trans.NoTrans,
                kStates, kStates, kStates,
                1.0, 0.0, transition, stateSpace.transitionOffset(t) >> 1, transitionLd,
                scratch, measurementTransitionBase >> 1, kStates,
                0.0, 0.0, scratch, innovationsTransitionBase >> 1, kStates);
            ZLAS.zcopy(kStates * kStates, nextMeasuredN, 0, 1,
                scratch, singleObservationTransitionBase, 1);
            computeConventionalStateAutocovariance(fr, result, t, kStates,
                scratch, innovationsTransitionBase,
                singleObservationTransitionBase,
                tmpBase);

            System.arraycopy(measuredN, 0, nextMeasuredN, 0, ksKs);
            ZLAS.zgemm(BLAS.Trans.NoTrans, BLAS.Trans.NoTrans,
                kStates, kStates, kStates,
                1.0, 0.0, measuredN, 0, kStates,
                transition, stateSpace.transitionOffset(t) >> 1, transitionLd,
            ...
            ZLAS.zgemm(BLAS.Trans.Trans, BLAS.Trans.NoTrans,
                ...
                1.0, 0.0, transition, stateSpace.transitionOffset(t) >> 1, transitionLd,
                scratch, tmpBase >> 1, kStates,
                0.0, 0.0, currentPredictedN, 0, kStates);
        }
    }
    ...
    private static void restoreMissingSquareEntries(KalmanSSM observationModel,
                                                    int t,
                                                    double[] source,
                                                    int sourceOffset,
                                                    int sourceLd,
    ...
    private static void addWithTranspose(double[] source, int sourceOffset,
                                             double[] target, int targetOffset,
                                             int n) {
        for (int i = 0; i < n; i++) {
    ...
    private static void transposeInPlace(double[] matrix, int offset, int n) {
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                int ij = offset + i * 2 * n + j * 2;
                int ji = offset + j * 2 * n + i * 2;
