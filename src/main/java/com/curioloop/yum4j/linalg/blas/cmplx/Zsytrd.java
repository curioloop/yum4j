...
interface Zsytrd {
    ...
    static int zsytrd(BLAS.Uplo uplo, int n, double[] A, int aOff, int lda,
        ...
        int nb = Ilaenv.ilaenv(1, "ZSYTRD", String.valueOf(uplo), n, 0, 0, -1);
        ...
        if (upper) {
            for (int i = n - 2; i >= 0; i--) {
                ...
                if (taur != 0.0 || taui != 0.0) {
                    ...
                    Zsymv.zsymv(BLAS.Uplo.Upper, i + 1, taur, taui,
                            A, aOff * 2, lda,
                            A, (aOff + i + 1) * 2, lda,
                            0.0, 0.0,
                            work, wOff, 1);

                        Zdot.zdotu(i + 1, work, wOff / 2, 1,
                            A, aOff + i + 1, lda, dotOut, 0);
                    ...
                    Zsyr2.zsyr2(BLAS.Uplo.Upper, i + 1, -1.0, 0.0,
                            A, (aOff + i + 1) * 2, lda,
                            work, wOff, 1,
                            A, aOff * 2, lda);
            ...
            for (int i = 0; i < n - 1; i++) {
                ...
                if (taur != 0.0 || taui != 0.0) {
                    ...
                    Zsymv.zsymv(BLAS.Uplo.Lower, m, taur, taui,
                            A, subStart, lda,
                            A, (aOff + (i + 1) * lda + i) * 2, lda,
                            0.0, 0.0,
                            work, wOff, 1);

                        Zdot.zdotu(m, work, wOff / 2, 1,
                            A, aOff + (i + 1) * lda + i, lda, dotOut, 0);
                    double alphaRe = -0.5 * (taur * dotOut[0] - taui * dotOut[1]);
                    double alphaIm = -0.5 * (taur * dotOut[1] + taui * dotOut[0]);
                    ...
                    Zsyr2.zsyr2(BLAS.Uplo.Lower, m, -1.0, 0.0,
                            A, (aOff + (i + 1) * lda + i) * 2, lda,
                            work, wOff, 1,
                            A, subStart, lda);
