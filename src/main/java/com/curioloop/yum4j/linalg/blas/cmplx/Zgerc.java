...
interface Zgerc {
     ...
     * @param lda    Leading dimension of the matrix A (must be at least max(1, m))
     */
    public static void zgerc(int m, int n, double alphaRe, double alphaIm,
                            double[] x, int xStart, int incX,
                            double[] y, int yStart, int incY,
                            double[] A, int aStart, int lda) {
        if (m <= 0 || n <= 0 || (alphaRe == 0.0 && alphaIm == 0.0)) return;
        if (incX == 1 && incY == 1) {
            for (int i = 0; i < m; i++) {
                int xp = xStart + i * 2;
                ...
                for (; j < n4; j += 4) {
                    int yp = yStart + j * 2;
                    int ap = rowOff + j * 2;
                    double y0r = y[yp], y0i = -y[yp+1];
                    double y1r = y[yp+2], y1i = -y[yp+3];
                    double y2r = y[yp+4], y2i = -y[yp+5];
                    double y3r = y[yp+6], y3i = -y[yp+7];
