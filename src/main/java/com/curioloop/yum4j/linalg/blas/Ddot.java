...
interface Ddot {
    ...
    static double ddot(int n, double[] x, int xOff, int incX,
        ...
        if (incX == 1 && incY == -1) {
            return ddotContiguousReverseY(x, xOff, y, yOff, n);
        }
        if (incX == -1 && incY == 1) {
            return ddotContiguousReverseY(y, yOff, x, xOff, n);
        }
    ...
    static double ddotContiguousReverseY(double[] x, int xOff, double[] y, int yOff, int n) {
        if (n <= THRESHOLD) {
            if (n == THRESHOLD && SIMD.supportDdot()) {
                return DdotSIMD.ddotReverseY(n, x, xOff, y, yOff);
            }
        ...
        int mid = n / 2;
        return ddotContiguousReverseY(x, xOff, y, yOff, mid) +
               ddotContiguousReverseY(x, xOff + mid, y, yOff - mid, n - mid);
    }
...
final class DdotSIMD {
    ...
    private static final VectorShuffle<Double> REVERSE = VectorShuffle.fromOp(SPECIES, i -> LANES - 1 - i);
    ...
    static double ddotReverseY(int n, double[] x, int xOff, double[] y, int yOff) {
        if (LANES == 4) {
            DoubleVector acc = DoubleVector.zero(SPECIES);
            int k = 0;
            for (; k + 3 < n; k += 4) {
                DoubleVector yv = DoubleVector.fromArray(SPECIES, y, yOff - k - 3).rearrange(REVERSE);
                acc = DoubleVector.fromArray(SPECIES, x, xOff + k).fma(yv, acc);
            }
            double s0 = acc.lane(0);
            double s1 = acc.lane(1);
            double s2 = acc.lane(2);
        ...
        if (LANES == 2) {
            ...
            int k = 0;
            for (; k + 3 < n; k += 4) {
                DoubleVector y01 = DoubleVector.fromArray(SPECIES, y, yOff - k - 1).rearrange(REVERSE);
                acc01 = DoubleVector.fromArray(SPECIES, x, xOff + k).fma(y01, acc01);
                DoubleVector y23 = DoubleVector.fromArray(SPECIES, y, yOff - k - 3).rearrange(REVERSE);
                acc23 = DoubleVector.fromArray(SPECIES, x, xOff + k + 2).fma(y23, acc23);
            }
            double s0 = acc01.lane(0);
            double s1 = acc01.lane(1);
            double s2 = acc23.lane(0);
